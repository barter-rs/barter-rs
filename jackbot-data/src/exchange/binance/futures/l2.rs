    ...
    async fn init(
        ...
        let instrument_map = instrument_map
            ...
            .map(|(sub_id, instrument_key)| {
                ...
                let sequencer = <BinanceFuturesUsdOrderBookL2Sequencer as L2Sequencer<
...
impl L2Sequencer<BinanceFuturesOrderBookL2Update> for BinanceFuturesUsdOrderBookL2Sequencer {
    fn new(snapshot_last_update_id: u64) -> Self {
        Self::new(snapshot_last_update_id)
    }
...
impl HasUpdateIds for BinanceFuturesOrderBookL2Update {
...
impl Canonicalizer for BinanceFuturesOrderBookL2Update {
    fn canonicalize(&self, timestamp: DateTime<Utc>) -> OrderBook {
        OrderBook::new(
            self.last_update_id,
            Some(self.time_engine),
            self.bids
                .iter()
...
mod tests {
    ...
    fn test_update_jackbot_order_book_with_sequenced_updates() {
        ...
        for (index, mut test) in tests.into_iter().enumerate() {
            ...
            {
                let jackbot_update = OrderBookEvent::Update(OrderBook::new(
                ...
                test.book.update(jackbot_update);
            }
