...
use jackbot_integration::{Validator, error::SocketError, protocol::websocket::WsMessage};
    ...
    Self: Clone + Default + Debug + for<'de> Deserialize<'de> + Serialize + Sized,
{
    ...
    /// sent over the [`WebSocket`](jackbot_integration::protocol::websocket::WebSocket). Implements
    ...
    /// [`WebSocket`](jackbot_integration::protocol::websocket::WebSocket) pings for the exchange
...
/// [`InstrumentKind`](jackbot_instrument::instrument::kind::InstrumentKind) market data on distinct servers,
...
/// [`WebSocket`](jackbot_integration::protocol::websocket::WebSocket) pings - used for exchanges
