//! Level 2 order book (full book) types and parsing for Bitget exchange.
...
use crate::{
    ...
    exchange::{
        Connector,
        bitget::{channel::BitgetChannel, market::BitgetMarket, spot::BitgetSpot},
...
/// Bitget HTTP OrderBook L2 snapshot url for spot market.
/// See: https://bitgetlimited.github.io/apidoc/en/spot/#get-orderbook
pub const HTTP_BOOK_L2_SNAPSHOT_URL_BITGET_SPOT: &str =
    "https://api.bitget.com/api/spot/v1/market/depth";

/// Sequencer implementation for Bitget Spot order book.
#[derive(Debug, Clone)]
pub struct BitgetSpotOrderBookL2Sequencer {
    pub last_update_id: u64,
    pub updates_processed: u64,
}

impl L2Sequencer<BitgetOrderBookL2Update> for BitgetSpotOrderBookL2Sequencer {
    ...
    fn validate_sequence(
        ...
        // Bitget doesn't provide sequence numbers - just apply all updates
...
#[derive(Debug)]
pub struct BitgetSpotOrderBooksL2SnapshotFetcher;

impl SnapshotFetcher<BitgetSpot, OrderBooksL2> for BitgetSpotOrderBooksL2SnapshotFetcher {
    fn fetch_snapshots<Instrument>(
        subscriptions: &[Subscription<BitgetSpot, Instrument, OrderBooksL2>],
...
#[derive(Debug)]
pub struct BitgetSpotOrderBooksL2Transformer<InstrumentKey> {
    instrument_map: Map<(InstrumentKey, BitgetSpotOrderBookL2Sequencer)>,
}

#[async_trait]
impl<InstrumentKey> ExchangeTransformer<BitgetSpot, InstrumentKey, OrderBooksL2>
    for BitgetSpotOrderBooksL2Transformer<InstrumentKey>
where
    ...
    async fn init(
        ...
        let instrument_map = instrument_map
            ...
            .map(|(sub_id, instrument_key)| {
                ...
                let sequencer = <BitgetSpotOrderBookL2Sequencer as L2Sequencer<
...
impl<InstrumentKey> Transformer for BitgetSpotOrderBooksL2Transformer<InstrumentKey>
where
    InstrumentKey: Clone,
{
...
/// Bitget real-time OrderBook Level2 update.
#[derive(Clone, PartialEq, PartialOrd, Debug, Deserialize, Serialize)]
...
mod tests {
    ...
    #[test]
    fn test_bitget_order_book_l2_update() {
        let input = r#"{
            "action": "update",
            "instId": "BTCUSDT",
            "bids": [["30000.0", "1.0"]],
    ...
    #[test]
    fn test_bitget_order_book_l2_snapshot() {
        let input = r#"{
            "action": "snapshot",
            "instId": "BTCUSDT",
            "bids": [["30000.0", "1.0"]],
