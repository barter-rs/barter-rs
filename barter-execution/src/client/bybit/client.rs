...
use super::websocket::BybitAccountStreamTransformer;
use super::{
    http::{
        parser::BybitParser,
        requests::{
...
use crate::client::bybit::websocket::{
    PingInterval, authenticate_connection, distribute_messages_to_exchange,
    schedule_pings_to_exchange, subscribe_topic,
};
use crate::error::{ApiError, OrderError, UnindexedApiError};
use crate::{
    ...
    error::{ClientError, UnindexedClientError, UnindexedOrderError},
    order::{
        Order, OrderKey,
        id::StrategyId,
        request::{
    ...
    Server: Clone + BybitServer + Sync,
{
    ...
    // TODO: Should we filter events based on the assets and instruments?
    async fn account_stream(
        ...
        authenticate_connection(&mut websocket, &self.credentials).await?;

        // Subscribe to topic
        subscribe_topic(&mut websocket).await?;
    ...
    async fn cancel_order(
        ...
        let state = response
            ...
            // Map error to the UnindexedOrderError
            .map_err(|err| match err {
                ...
                _else => {
                    UnindexedOrderError::Rejected(UnindexedApiError::Custom(_else.to_string()))
                }
            })
            // Some errors can be interpreted as `order is already canceled`. In
            // these cases we return Ok with cancellation.
            .or_else(|err| {
                if let OrderError::Rejected(api_error) = &err {
                    if matches!(
                        api_error,
                        ApiError::OrderAlreadyCancelled | ApiError::OrderNotFound
                    ) {
                        return Ok(Cancelled {
                            id: cancel_request
                                .state
