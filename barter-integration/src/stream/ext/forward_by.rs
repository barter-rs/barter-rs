...
impl<S, A, B, FnPredicate, FnForward, FwdErr> Stream for ForwardBy<S, FnPredicate, FnForward>
where
    S: Stream,
...
mod tests {
    ...
    #[tokio::test]
    async fn test_forward_by_with_custom_error_type() {
        let waker = futures::task::noop_waker_ref();
        let mut cx = Context::from_waker(waker);
        ...
        let mut stream = rx.forward_by(
            ...
            |item: i32| -> Result<(), String> {
                if item == 4 {
                    Err(format!("rejected {item}"))
                } else {
                    Ok(())
                }
            },
