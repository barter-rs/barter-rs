...
mod tests {
    ...
    async fn test_merge() {
        ...
        let (left_tx, left_rx) = unbounded_channel::<&'static str>();
        let (right_tx, right_rx) = unbounded_channel::<&'static str>();

        let mut stream = merge(
            UnboundedReceiverStream::new(left_rx),
            UnboundedReceiverStream::new(right_rx),
        );
