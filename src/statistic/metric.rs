...
impl<T> TradingStatistics<T> where T: MetricRolling + Clone {
    pub fn new(summary: HashMap<SymbolID, T>, tear_sheet: Option<HashMap<SymbolID, HashMap<MetricID, T>>>) -> Self {
        Self {
            summary,
            tear_sheet
        }
...
pub struct TradingStatisticsBuilder<T> where T: MetricRolling + Clone {
    ...
    summary_generation_func: Option<fn(&Vec<SymbolID>, &Vec<Position>, T) -> HashMap<SymbolID, T>>,
...
impl<T> TradingStatisticsBuilder<T> where T: MetricRolling + Clone {
    ...
    pub fn summary(mut self, summary_init: T, func: fn(&Vec<SymbolID>, &Vec<Position>, T) -> HashMap<SymbolID, T>) -> Self {
