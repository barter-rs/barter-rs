use crate::{
    engine::{
        Engine, audit::context::EngineContext, clock::EngineClock, error::UnrecoverableEngineError,
    },
    strategy::{on_disconnect::OnDisconnectStrategy, on_trading_disabled::OnTradingDisabled},
};
use barter_integration::{Terminal, collection::none_one_or_many::NoneOneOrMany};
...
impl<Clock, State, ExecutionTxs, Strategy, Risk> Auditable
...
/// AuditTick kind generated by running an `Engine` for a single tick.
#[derive(Debug, Clone, Eq, PartialEq, Ord, PartialOrd, Hash, Deserialize, Serialize)]
pub enum EngineAudit<Input, Output> {
    /// Input event feed ended.
    FeedEnded,
    /// `Engine` processed an `Event`.
    Process(ProcessAudit<Input, Output>),
}

#[derive(Debug, Clone, Eq, PartialEq, Ord, PartialOrd, Hash, Deserialize, Serialize)]
pub struct ProcessAudit<Input, Output> {
    pub input: Input,
    pub output: Output,
} // where Output = EngineOutputs<Output>

pub struct EngineOutputs<Output> {
    pub outputs: NoneOneOrMany<Output>,
    pub errors: NoneOneOrMany<UnrecoverableEngineError>,
} // where Output = EngineOutput
