use crate::{
    engine::{
        ...
        action::{ActionOutput, generate_algo_orders::GenerateAlgoOrdersOutput},
...
pub enum EngineAuditNew<Input, Output> {
    /// Input event feed ended.
    FeedEnded,
    /// `Engine` processed an `Event`.
    Process(ProcessAuditNew<Input, Output>),
}

pub struct ProcessAuditNew<Input, Output> {
    pub input: Input,
    pub output: Output,
} // where Output = NewEngineOutputConcept<Output>
