...
impl<Clock, Strategy, Risk, MarketStream> SystemBuilder<Clock, Strategy, Risk, MarketStream> {
            ...
            + 'static,
    {
        ...
        let (engine, audit_rx) = match (engine_feed_mode, audit_mode) {
            (EngineFeedMode::Sync, AuditMode::Enabled) => {
            ...
            (EngineFeedMode::Sync, AuditMode::Disabled) => {
                let handle = runtime.spawn_blocking(move || {
            ...
            (EngineFeedMode::Async, AuditMode::Enabled) => {
                // Initialise Audit channel
                let (audit_tx, audit_rx) = mpsc_unbounded();
                let mut audit_tx = ChannelTxDroppable::new(audit_tx);
            ...
            (EngineFeedMode::Async, AuditMode::Disabled) => {
                let handle = runtime.spawn(async move {
