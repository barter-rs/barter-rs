...
/// Market data state that holds a full orderbook. This state can later be used
/// in a strategy to make decisions on.
#[derive(Debug, Default, Clone)]
struct MarketData {
    pub book: OrderBook,
}

impl<InstrumentKey> MarketDataState<InstrumentKey> for MarketData {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let state = EngineState::<MarketData, DefaultStrategyState, DefaultRiskManagerState>::builder(
        &instruments,
    )
