...
pub struct CustomRiskManager<State> {
    pub max_notional_per_instrument: CheckHigherThan<Decimal>,
...
impl<State> Default for CustomRiskManager<State> {
    fn default() -> Self {
        Self {
            max_notional_per_instrument: MAX_USDT_NOTIONAL_PER_ORDER,
    ...
    fn check(
        ...
        let (approved_opens, refused_opens): (Vec<_>, Vec<_>) = opens
            ...
            .fold((Vec::new(), Vec::new()), |(mut approved, mut refused), request_open| {
                ...
                if let Err(error) = self.max_notional_per_instrument.check(&notional) {
                    warn!(
